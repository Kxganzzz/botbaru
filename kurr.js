...
if (!mek.key.remoteJid.endsWith('@g.us') && offline){
		...
		const reply = (teks) => {
			kurr.sendMessage(from, teks, text, { thumbnail: dfrply, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jmn} - ${week}  - ${calender}`,body:"KurrXdゑ v.3.0",previewType:"PHOTO",thumbnail:ofrply,sourceUrl:`https://wa.me/62881037046507`}}})
		}
...
cpref = `Hayo Ngomongin Yudi Yaaa Aowkowkwoo`
reply(cpref)
}
if(budy.includes('yudi')){
cpref = `Hayo Ngomongin Yudi  Yaaa Aowkowkwoo`
...
_え ${prefix}othermenu_
 `
sendButLocation(from, `${menu}`, "*_© YudiXdゑ v.3.0_*", {jpegThumbnail:ofrply1,name:""}, [{buttonId:`info`,buttonText:{displayText:'INFO BOT'},type:1},{buttonId:`store`,buttonText:{displayText:'STORE BOT'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
...
_え ${prefix}nano [ _nama file_ ]_             
 `
sendButLocation(from, `${menu}`, "*_© YudiXdゑ v.3.0_*", {jpegThumbnail:ofrply1,name:""}, [{buttonId:`menu`,buttonText:{displayText:'BACK TO MENU'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
else if(menusimpel = true){
        stod = `${sender}`
fill =`Hai Kak ${pushname}, ${tampilUcapan}
...
-CALL BOT
-VC BOT
-MASUKIN BOT KE GRUP, WA OWNER .OWNER
LANGGAR BANNED
...
YOUTUBE OWNER:https://youtube.com/c/yudipermanaff
`
sendButLocation(from, `${menu}`, "*_© YudiXdゑ v.3.0_*", {jpegThumbnail:ofrply1,name:""}, [{buttonId:`ping`,buttonText:{displayText:'SPEED BOT'},type:1},{buttonId:`sewabot`,buttonText:{displayText:'SEWABOT'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
else if(menusimpel = true){
        stod = `${sender}`
fill =`Hai Kak ${pushname}, ${tampilUcapan}
...
case 'catlog': 
  if (!isRegistered) return reply(`daftar dlu om ketik .verify`)
                ...
                nama = ' ゑwelcome to yudi storeゑ '
       ownerJid = "62881037046507@s.whatsapp.net"
                nomor = 1
                stod = `${sender}`
       stst = await kurr.getStatus(`${sender.split('@')[0]}@c.us`)
				stst = stst.status == 401 ? '' : stst.status
       menunya = `*YudiXd Melayani Jasa Pembelian Diamond/Run Bot*
*Jasa Run Bot/ Hosting Bot*
*Dengan Harga Terjangkau*
*_List Harga Tanya Owner_*
Minat .owner Aja Gass Beli Om
Run Via Heroku

INFO/SARAN: JANGAN BELI YANG DM DIKIT
KARNA YANG MURAH YANG DI ATAS 60 DIAMOND

Dm Dihabiskan= NO GARANSI
*_List Harga Diamond_*
Chat Owner Aja Ragu? Jangan Order
Minat Pm .owner

*GARANSI 3HARI APA BILA SETELAH PEMBAYARAN TIDAK MASUK*

TOP UP GEGE ANTI DM MERAH MURAH LAGI 
KAPAN LAGI OMMM ORDER SKRNG YA KAPAN LAGI
BAYAR VIA DANA,CASH,GOPAY GASS OM
...
gifnya = await getBuffer('https://telegra.ph/file/457d3be9baecef635c357.mp4')
		anu =`*YudiXd Melayani Jasa Pembelian Diamond/Run Bot*
*Jasa Run Bot/ Hosting Bot*
*Dengan Harga Terjangkau*
*_List Harga Tanya Owner_*
Minat .owner Aja Gass Beli Om
